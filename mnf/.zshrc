...
source <(fzf --zsh)
# --- Triple-check safeguard for `rm -rf` ---
# Wrap rm in a function
rm() {
  # Detect if the arguments include -rf or -fr as a combined recursive/force deletion
  if [[ "$*" == *"-rf"* || "$*" == *"-fr"* ]]; then
    ...
    # If all confirmations passed, run real rm
    command rm "$@"
  else
